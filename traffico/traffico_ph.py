...
class CGI_CLI(object):
    ...
    def init_cgi(chunked = None, css_style = None, newline = None, \
        ...
        if CGI_CLI.remote_addr and CGI_CLI.http_user_agent and not CGI_CLI.json_api:
    ...
    def uprint_json_results(msg = None, raw_log = None, sort_keys = None,\
        ...
        print_text = None
        if msg and not CGI_CLI.uprint_json_results_printed:
...
def read_bgp_data_json_from_logfile(filename = None, printall = None):
    ...
    with open(filename,"r") as fp:
        text = fp.read()
    if text:
        try: bgp_data_json_text = text.split('return_bgp_data_json()')[1].strip()
...
try:
    ...
    CGI_CLI.JSON_RESULTS['success'] = str()
    ...
    changelog = 'https://github.com/peteneme/pyxWorks/commits/master/traffico/traffico_ph.py'

    SCRIPT_NAME = 'TRAFFIC ON/OFF TOOL'
    ...
    if CGI_CLI.cgi_active and len(device_list) == 0 and\
        ...
        if (CGI_CLI.data.get("script_action") or CGI_CLI.data.get("shut") or CGI_CLI.data.get("noshut") or CGI_CLI.data.get("radio")):
            ...
            text = 'Please specify --shut or --noshut ... '
    ...
    if SCRIPT_ACTION == str():
        text = 'Action (shut/noshut) NOT SPECIFIED!'
    ...
    for device in device_list:
        if device:
            ...
            CGI_CLI.uprint("\nSCRIPT_ACTION = %s, DEVICE = %s, ROUTER_TYPE: %s%s\n" % \
                (SCRIPT_ACTION, str(device), RCMD.router_type, \
                ', SIMULATION MODE = ON' if CGI_CLI.data.get("sim") else str()))

            ### def READ SHUT LOGFILE ###
            if logfilename and SCRIPT_ACTION == 'noshut':
                ...
                if not last_shut_file:
                    RCMD.disconnect()
                    sys.exit(0)
                bgp_data = read_bgp_data_json_from_logfile(last_shut_file, printall = printall)
                if not bgp_data:
                    text = " ... Please insert valid shut session log! \nFile " + last_shut_file + \
                        " \ndoes not contain return_bgp_data_json !"
                    CGI_CLI.uprint(text, color = 'red', tag = 'h1')
                    CGI_CLI.JSON_RESULTS['errors'] += '[%s] ' % (text)
                    RCMD.disconnect()
                    sys.exit(0)
            ...
            ### def END IF ON LOCAL_AS_NUMBER #################################
            if LOCAL_AS_NUMBER:
                CGI_CLI.uprint('\nLOCAL_AS_NUMBER = %s' % (LOCAL_AS_NUMBER))
            else:
                text = "PROBLEM TO PARSE LOCAL AS NUMBER!"
                CGI_CLI.uprint(text, color = 'red', tag = 'h1')
                CGI_CLI.JSON_RESULTS['errors'] += '[%s] ' % (text)
                RCMD.disconnect()
                sys.exit(0)
            ...
            ### def JUNOS BGP NEIGHBORS PARSING #######################################
                ...
                ### IMN ###
                if LOCAL_AS_NUMBER == '2300':
                    CGI_CLI.uprint('NOT IMPLEMENTED YET !', tag ='h1', color = 'red')


            ### def HUAWEI BGP NEIGHBORS PARSING ######################################
            elif RCMD.router_type == 'huawei':
                ipv4_list, dummy = huawei_parse_bgp_summary(rcmd_outputs[0], LOCAL_AS_NUMBER)
                dummy, ipv6_list = huawei_parse_bgp_summary(rcmd_outputs[2], LOCAL_AS_NUMBER)
                ...
                if LOCAL_AS_NUMBER == '2300':
                    if SCRIPT_ACTION == 'shut':
                        vpn_list = []
                        try:
                            for vpn_part in rcmd_outputs[3].split('VPN-Instance')[1:]:
            ...
            if bgp_config == [] or bgp_data == {}:
                CGI_CLI.uprint('VOID CONFIG, DO SET/CLEAR OVERLOAD BIT ONLY.', tag = 'h3' , color = 'blue')
                LCMD.eval_command('return_bgp_data_json()')
            elif CGI_CLI.data.get("printall"):
                CGI_CLI.uprint(bgp_data, name = 'bgp_data', jsonprint = True)
