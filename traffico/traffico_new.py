...
try:
    ...
    class RCMD(object):
        ...
        def get_json_from_vision(URL = None):
            ...
            if RCMD.USERNAME and RCMD.PASSWORD:
                ...
                RCMD.vision_api_json_string = LCMD.run_command(local_command, printall = None, ommit_logging = True)
        ...
        def run_command(cmd_line = None, printall = None,
            ...
            if sys.version_info.major <= 2:
                ...
                if cmd_line:
                    ...
                    if CGI_CLI.cgi_active and not ommit_logging:
            ...
            if CGI_CLI.cgi_active and not ommit_logging:
                CGI_CLI.logtofile('\n<pre>' + \
                    ...
                    '\n</pre>\n', raw_log = True)
            elif not ommit_logging: CGI_CLI.logtofile(os_output + '\n')
            return os_output
            ...
            if len(cmd_list)>0:
                ...
                for cmd_line in cmd_list:
                    ...
                    if CGI_CLI.cgi_active:
                        ...
                        CGI_CLI.logtofile('LOCAL_COMMAND: ' + str(cmd_line) + '\n')
                    try:
                        os_output_raw = subprocess.check_output(cmd_line, stderr=subprocess.STDOUT, shell=True)
                        os_output = os_output_raw.decode()
                    except (subprocess.CalledProcessError) as e:
    ...
    if CGI_CLI.cgi_active and not CGI_CLI.submit_form:
        ...
        ### READ SCRIPT ACTION ###
        if (CGI_CLI.data.get("script_action") or CGI_CLI.data.get("radio") \
            or CGI_CLI.data.get("noshut") or CGI_CLI.data.get("shut")):
            if CGI_CLI.data.get("shut") or CGI_CLI.data.get("radio") == "shut":
                SCRIPT_ACTION = 'shut'
            elif CGI_CLI.data.get("noshut") or CGI_CLI.data.get("radio") == "noshut":
                SCRIPT_ACTION = 'noshut'
        else:
    ...
    if device:
        ...
        CGI_CLI.set_logfile(logfilename)
        ...
        if logfilename and SCRIPT_ACTION == 'noshut':
            ...
            if not bgp_data:
                CGI_CLI.uprint( " ... Please insert valid shut session log! \nFile " + last_shut_file + \
                    " \ndoes not contain return_bgp_data_json !", color = 'red', tag = 'h1')
                RCMD.disconnect()
                sys.exit(0)

        ### ROUTER TYPE + SIM PRINT ###
        CGI_CLI.uprint("ROUTER_TYPE: %s %s" % (RCMD.router_type, \
            '\nSIMULATION MODE = ON' if CGI_CLI.data.get("sim") else str()), color = 'blue')
        ...
        if LOCAL_AS_NUMBER:
            CGI_CLI.uprint(LOCAL_AS_NUMBER, name = True , color = 'blue')
        else:
            CGI_CLI.uprint("PROBLEM TO PARSE LOCAL AS NUMBER!" , color = 'red', tag = 'h1')
            RCMD.disconnect()
            sys.exit(0)
        ...
        if bgp_config == [] or bgp_data == {}:
            CGI_CLI.uprint('VOID CONFIG, END!', tag = 'h1' , color = 'red')
    ...
    if traceback_found:
        CGI_CLI.send_me_email( \
            subject = 'TRACEBACK-TRAFFICO-' + logfilename.replace('\\','/').\
