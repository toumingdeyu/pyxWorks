...
def main():
  ...
  if nso_auth_data and json_raw_data:
    ...
    json_config=get_json_element(json_raw_data,'config')
    print(json_config)
    if not json_config and not nso_device: exit(0)
    ...
    response = requests.patch(uri, auth=auth, headers=restconf_headers, data=json.dumps(json_config))
