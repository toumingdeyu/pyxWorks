...
def device_command(self, uinfo, input, cmd, get_platform = True):
    """
    ...
    if input_device:
        ...
        with m.start_read_trans(usid=uinfo.usid) as t:
            ...
            dev = root.ncs__devices.device[input_device]
            ...
            try:
                ...
                self.log.info("\nEXCEPTION in REMOTE_COMMAND({}):{}\n{}".format(dev.platform.name, str(cmd), str(E)))
        m.close()
    else: self.log.info("\nMissing input device in REMOTE_COMMAND()!")
    ### DEVICE_COMMAND END ###
    if get_platform: return result, platform
    else: return result
