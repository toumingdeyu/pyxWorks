...
class RCMD_class():
    ...
    def __init__(self, device=None, **kwargs):
        ...
        if self.input:
            if self.input.device:
                self.input_device = self.input.device
            elif self.input.ip:
                self.input_device = self.input.ip
            elif self.input.device_iap.device:
                self.input_device = self.input.device_iap.device
        ...
        if self.input_device and self.uinfo:
            ...
            if self.dev.platform.name:
                ...
                try:
                    self.ip = self.root.ncs__devices.device[self.input_device].address
                except:
                    pass

                try:
                    self.port = self.root.ncs__devices.device[self.input_device].port
                except:
                    pass
                ...
                if self.dev.platform.name == "ios-xe":
                    self.router_type = 'cisco_ios'
                elif self.dev.platform.name == "ios-xr":
                    self.router_type = 'cisco_xr'
                elif self.dev.platform.name == "junos":
                    self.router_type = 'juniper'
                elif self.dev.platform.name == "huawei-vrp":
                ...
                if self.os_type == "ios-xe":
                    try:
                        self.sw_version = str(result.split('Software, Version')[1]\
                            .split()[0].split('[')[0].strip())
                    except:
                        try:
                            self.sw_version = str(result.split('Version      :')[1]\
                                .split()[0].split('[')[0].strip())
                        except: pass
                    try:
                        self.hw_type = str(result.split(') processor')[0]\
                    ...
                    if 'IOS-XRv 9000' in self.hw_type or 'NCS' in self.hw_type:
                        self.x64 = True
                    elif 'IOS XRv Series' in self.hw_type:
                        self.x64 = False
                    ...
                    try:
                        ...
                        pass
                    try:
                        self.hw_type = str(result.split(' version information:')[0]\
                            .splitlines()[-1].strip())
                    except:
                        pass
    ...
    def run_commands(self, cmd, **kwargs):
        ...
        if isinstance(cmd, (basestring)):
            ...
            if cmd_data.get('cisco_ios'):
                cmd_data["ios-xe"] = cmd_data.get('cisco_ios')
            if cmd_data.get('cisco_xr'):
                cmd_data["ios-xr"] = cmd_data.get('cisco_xr')
            if cmd_data.get('juniper'):
                cmd_data["junos"] = cmd_data.get('juniper')
            if cmd_data.get('huawei'):
                cmd_data["huawei-vrp"] = cmd_data.get('huawei')
        ...
        try:
            ...
            if self.dev.platform.name == "ios-xe" and cmd_data.get("ios-xe"):
                ...
                if isinstance(cmd_data.get("ios-xe"), (list, tuple)):
                    ...
                    command.args = [cmd_data.get("ios-xe", str())]
            ...
            ### Cisco XR platform #############################################
                ...
                if isinstance(cmd_data.get("ios-xr"), (list, tuple)):
                    ...
                    command.args = [cmd_data.get("ios-xr", str())]

                command_output = self.dev.live_status.cisco_ios_xr_stats__exec.any(command)
                result = command_output.result
            ...
            ### Huawei platform ###############################################
                ...
                if isinstance(cmd_data.get("huawei-vrp"), (list, tuple)):
                    ...
                    command.args = [cmd_data.get("huawei-vrp", str())]
            ...
            ### Juniper platform ##############################################
                ...
                command_input = self.dev.rpc\
                    .jrpc__rpc_request_shell_execute.request_shell_execute.get_input()

                if isinstance(cmd_data.get("junos"), (list, tuple)):
                    for item in cmd_data.get("junos"):
                        command_input.command = 'cli ' + item.replace('|', r'\|')
                        command_output = self.dev.rpc.jrpc__rpc_request_shell_execute\
                            .request_shell_execute.request(command_input)
                        result = result + '\n' + command_output.output
                else:
                    command_input.command = ['cli ' + cmd_data.get("junos", str())\
                        .replace('|', r'\|')]
                    command_output = self.dev.rpc.jrpc__rpc_request_shell_execute\
                        .request_shell_execute.request(command_input)
