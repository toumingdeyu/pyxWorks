...
class CGI_CLI(object):
    ...
    def init_cgi(interaction = None, os_environ_set = None, return_http_status = None):
        ...
        if CGI_CLI.cgi_active:
            ...
            if CGI_CLI.return_http_status:
                print("Retry-After: 300")            
                #CGI_CLI.print_or_buffer("Status: %s %s\r\n" % ('200',""))
            CGI_CLI.print_or_buffer('\r\n\r\n')    
            CGI_CLI.print_or_buffer("<html><head><title>%s</title></head><body>" % 
    ...
    def uprint(text, tag = None, tag_id = None, color = None, name = None, jsonprint = None):
        ...
        if CGI_CLI.debug:
            ...
            if CGI_CLI.cgi_active:
                ...
                if tag: CGI_CLI.print_or_buffer('<%s%s%s>'%(tag,' id="%s"'%(tag_id) if tag_id else str(),' style="color:%s;"'%(color) if color else 'black'))
                if isinstance(print_text, six.string_types):
                    print_text = str(print_text.replace('&','&amp;').replace('<','&lt;'). \
                        replace('>','&gt;').replace(' ','&nbsp;').replace('"','&quot;').replace("'",'&apos;').\
                        replace('\n','<br/>'))
            CGI_CLI.print_or_buffer(print_name + print_text)
            del print_text
            if CGI_CLI.cgi_active: 
                if tag: CGI_CLI.print_or_buffer('</%s>'%(tag))
                else: CGI_CLI.print_or_buffer('<br/>');
    ...
    def formprint(form_data = None, submit_button = None, pyfile = None, tag = None, color = None):
        ...
        def subformprint(data_item):
            if isinstance(data_item, six.string_types): CGI_CLI.print_or_buffer(data_item)
            elif isinstance(data_item, (dict,collections.OrderedDict)):
                if data_item.get('raw',None): CGI_CLI.print_or_buffer(data_item.get('raw'))
                elif data_item.get('textcontent',None): 
                    CGI_CLI.print_or_buffer('<textarea type = "textcontent" name = "%s" cols = "40" rows = "4"></textarea>'%\
                        (data_item.get('textcontent')))
                elif data_item.get('text'):
                    CGI_CLI.print_or_buffer('%s: <input type = "text" name = "%s"><br />'%\
                        (data_item.get('text','').replace('_',' '),data_item.get('text')))
                elif data_item.get('radio'):    
                    CGI_CLI.print_or_buffer('<input type = "radio" name = "%s" value = "%s" /> %s'%\
                        (data_item.get('radio'),data_item.get('radio'),data_item.get('radio','').replace('_',' ')))
                elif data_item.get('checkbox'):
                    CGI_CLI.print_or_buffer('<input type = "checkbox" name = "%s" value = "on" /> %s'%\
                        (data_item.get('checkbox'),data_item.get('checkbox','').replace('_',' ')))
                elif data_item.get('dropdown'):
                    CGI_CLI.print_or_buffer('<select name = "dropdown[%s]">'%(data_item.get('dropdown')))
                    for option in data_item.get('dropdown').split(','):
                        CGI_CLI.print_or_buffer('<option value = "%s" selected>%s</option>')%(option,option)
                    CGI_CLI.print_or_buffer('</select>')
                elif data_item.get('file'):
                   CGI_CLI.print_or_buffer('Upload file: <input type = "file" name = "file[%s]" />'%(data_item.get('file').replace('\\','/')))  
                elif data_item.get('submit'):    
                    CGI_CLI.print_or_buffer('<input id = "%s" type = "submit" name = "submit" value = "%s" />'%\
        ...
        if CGI_CLI.cgi_active:
            CGI_CLI.print_or_buffer('<br/>');
            if tag and 'h' in tag: CGI_CLI.print_or_buffer('<%s%s>'%(tag,' style="color:%s;"'%(color) if color else str()))
            if color or tag and 'p' in tag: tag = 'p'; CGI_CLI.print_or_buffer('<p%s>'%(' style="color:%s;"'%(color) if color else str()))
            CGI_CLI.print_or_buffer('<form action = "/cgi-bin/%s" enctype = "multipart/form-data" action = "save_file.py" method = "post">'%\
                (i_pyfile))
            ### RAW HTML ###
            if isinstance(form_data, six.string_types): CGI_CLI.print_or_buffer(form_data)
            ...
            CGI_CLI.print_or_buffer('</form>')
            if tag and 'p' in tag: CGI_CLI.print_or_buffer('</p>')
            if tag and 'h' in tag: CGI_CLI.print_or_buffer('</%s>'%(tag))
